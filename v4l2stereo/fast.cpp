...
int fast::update(unsigned char* img, int img_width, int img_height, int desired_features, int camera_index)
{
	int i,n;
	...
	detect_nonmax(img_mono, img_width, img_height, img_width, threshold[camera_index], &ret_num_corners);
	ret_num_corners = num_nonmax;
	if (ret_num_corners < desired_features-10) {
		threshold[camera_index] -= 2;
		if (threshold[camera_index] < 20) threshold[camera_index]=20;
	}
	if (ret_num_corners > desired_features+10) {
		threshold[camera_index] += 2;
		if (threshold[camera_index] > 200) threshold[camera_index]=200;
	}
...
/* shows FAST corners */
void fast::show(
    unsigned char *outbuf, int img_width, int img_height)  /* output image.  Note that this is YUVY */
{
    int f,x,y,n;
