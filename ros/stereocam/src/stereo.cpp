...
int svs::update_sums(int cols, /* if non-zero we're dealing with columns not rows */
    ...
    if (enable_segmentation) {
        ...
        if (cols == 0) {
            ...
            for (j = SVS_PEAK_WIDTH; j < max - SVS_PEAK_WIDTH - 1; j++) {
                if (row_peaks[j] > av_peaks) {
                    for (k = -1; k <= 1; k++) {
...
void svs::non_max(int cols, /* if non-zero we're dealing with columns not rows */
    ...
    max -= SVS_PEAK_WIDTH;
    for (i = SVS_PEAK_WIDTH; i < max; i++) {
        if (row_peaks[i] > max_peaks) max_peaks = row_peaks[i];
    }
    unsigned int threshold = (unsigned int)((int)max_peaks * (int)min_response / 100);
...
int svs::compute_descriptor(
    int px, int py, unsigned char* rectified_frame_buf,
